...
- (void)startProcessing
{
    ...
    __weak typeof(self) weakSelf = self;
        for (int i = 1; i < n; i++) {
            ...
            NSMutableArray *samples = [self generateSamplesWithTrack:_composition];
            [self encodeSampleBuffer:samples];
...
- (NSMutableArray* )generateSamplesWithTrack:(AVAsset *)asset
{
    ...
    while((sample = [readerOutput copyNextSampleBuffer])) {
        ...
        NSLog(@"count = %lu",(unsigned long)samples.count);
        CFRelease(sample);
    }
    if (samples.count > 0 ) {
        self.intervalTime = CMTimeMakeWithSeconds(CMTimeGetSeconds(self.segDuration)/(float)(samples.count), _asset.duration.timescale);
    }
    return samples;
}

- (void)encodeSampleBuffer:(NSMutableArray *)samples
{
    for(NSInteger i = 0; i < samples.count; i++) {
        ...
        size_t index = samples.count - i - 1;
        
        if (0 == _frame_count) {
            presentationTime = kCMTimeZero;
            index = samples.count - i - 2; //倒过来的第一帧是黑的丢弃
        }
        CMTimeShow(presentationTime);
        
        
        CVPixelBufferRef imageBufferRef = CMSampleBufferGetImageBuffer((__bridge CMSampleBufferRef)samples[index]);
